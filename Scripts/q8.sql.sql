...
SELECT p.park_name, hg.park, team, sum(hg.attendance)/games as avg_attendance
FROM homegames as hg
LEFT JOIN parks AS p ON hg.park =p.park
Where games >= 10
and year = 2016
group by park_name, team, hg.attendance, games, year, hg.park
order by avg_attendance desc
Limit 5;
