...
Select DISTINCT(CONCAT(namefirst, ' ', namelast)) as player_name, Sum(salary::int::money) as total_majorleague_salary, sch.schoolname 
FROM people as p 
Left join salaries as s on p.playerid =s.playerid
left join (Select distinct playerid, schoolid from collegeplaying) as cp on p.playerid = cp.playerid
